...
<body class="font-poppins text-dark text-sm leading-loose">
        ...
        <div class="container">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                            ...
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    ...
                                    <div class="swiper-slide">
                                        ...
                                        <img class="w-full h-full" src="{{ $picture->link }}" alt="product image" loading="lazy" width="432" height="480">
                ...
                <div class="lg:col-span-3">
                    ...
                    <p class="mb-8">{{ $car->seller_comment}}</p>
                        ...
                        <div class="mb-8">
                            <div class="flex flex-wrap items-center mt-8">
                                <form action="{{ route('create.order') }}" method="POST">
                                    ...
                                    <button type="submit" class="w-full inline-block align-middle bg-dark leading-none py-5 px-5 md:px-8 text-sm text-white transition-all hover:bg-dark uppercase font-semibold hover:text-white">Place
                                        Order</button>
    ...
    <@foreach ($saleCategories as $saleCategory) <section class="product-section pt-2 bg-light">
        <div class="container">
                                        ...
                                        <a href="{{ route('car', ['car' => $car->id]) }}">
                                            <div class="border border-solid border-gray-300 transition-all hover:shadow-product group">
                                                <div id="tab1" class="relative overflow-hidden">
                                                    <span class="font-medium uppercase text-sm text-black inline-block py-1 px-2 leading-none absolute top-3 left-3">{{ $car->availability_status }}</span>
                                                    <button class="px-3 py-2 font-bold text-white absolute bottom-0 right-0" style="background-color: rgb(220 38 38);">{{ $car->currency->code }}
                                                        {{ number_format($car->price, 2) }}</button>
                                                    <img class="w-full h-full" src="{{ $car->car_pictures->count() ? $car->car_pictures[0]->link : asset('images/featured-vehicles/v1.jpg') }}" alt="product image" loading="lazy" width="432" height="480" />
                                                </div>
                                                ...
                                                <div class="flex justify-between px-4 py-5">
                                                    <div class="mr-3"><i class="bi bi-calendar-check-fill mr-2"></i>{{ $car->manufacture_year }}
                                                    </div>
                                                    <div class="mr-3">
                                                        <form action="{{ route('create.order') }}" method="POST">
                                                            <input type="hidden" name="date" value={{ date('Y-m-d') }} />
                                                            ...
                                                            <input type="hidden" value="{{ $car->id }}" name="car_id">
                                                            @endauth
                                                            <button type="submit" class="inline-block align-middle bg-dark leading-none py-2 px-2 md:px-3 text-sm text-white transition-all hover:bg-dark uppercase hover:text-white">Place
                                                                Order</button>
